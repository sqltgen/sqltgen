...
object Queries {
    ...
    fun getTopSellingBooks(conn: Connection): List<GetTopSellingBooksRow> {
        conn.prepareStatement(SQL_GETTOPSELLINGBOOKS).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                while (rs.next()) rows.add(GetTopSellingBooksRow(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getBigDecimal(4), rs.getObject(5)))
            }
            return rows
        }
    }
    ...
    fun getBestCustomers(conn: Connection): List<GetBestCustomersRow> {
        conn.prepareStatement(SQL_GETBESTCUSTOMERS).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                while (rs.next()) rows.add(GetBestCustomersRow(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getObject(4)))
            }
            return rows
        }
    }
