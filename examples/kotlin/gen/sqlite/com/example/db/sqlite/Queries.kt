...
object Queries {
    ...
    fun getUser(conn: Connection, id: Int): Users? {
        conn.prepareStatement(SQL_GETUSER).use { ps ->
            ps.setInt(1, id)
            ps.executeQuery().use { rs ->
                if (!rs.next()) return null
                return Users(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4))
            }
        }
    }
    ...
    fun listUsers(conn: Connection): List<Users> {
        conn.prepareStatement(SQL_LISTUSERS).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                while (rs.next()) rows.add(Users(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4)))
            }
            return rows
        }
    }
    ...
    fun deleteUser(conn: Connection, id: Int): Unit {
    ...
    fun createPost(conn: Connection, userId: Int, title: String, body: String?): Unit {
    ...
    fun listPostsByUser(conn: Connection, userId: Int): List<ListPostsByUserRow> {
        conn.prepareStatement(SQL_LISTPOSTSBYUSER).use { ps ->
            ps.setInt(1, userId)
            val rows = mutableListOf<ListPostsByUserRow>()
            ps.executeQuery().use { rs ->
                while (rs.next()) rows.add(ListPostsByUserRow(rs.getInt(1), rs.getString(2), rs.getString(3)))
            }
            return rows
        }
    }
    ...
    fun listPostsWithAuthor(conn: Connection): List<ListPostsWithAuthorRow> {
        conn.prepareStatement(SQL_LISTPOSTSWITHAUTHOR).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                while (rs.next()) rows.add(ListPostsWithAuthorRow(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4)))
            }
            return rows
        }
    }
    ...
    fun getActiveAuthors(conn: Connection): List<GetActiveAuthorsRow> {
        conn.prepareStatement(SQL_GETACTIVEAUTHORS).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                while (rs.next()) rows.add(GetActiveAuthorsRow(rs.getInt(1), rs.getString(2), rs.getString(3)))
            }
            return rows
        }
    }
