#pragma once
// Generated by sqltgen. Do not edit.

#include <cstdint>
#include <optional>
#include <stdexcept>
#include <string>
#include <vector>

#include <mysql/mysql.h>

#include "author.hpp"
#include "book.hpp"

struct ListBooksWithAuthorRow {
    std::int64_t id;
    std::string title;
    std::string genre;
    std::string price;
    std::optional<std::string> published_at;
    std::string author_name;
    std::optional<std::string> author_bio;
};

struct GetTopSellingBooksRow {
    std::int64_t id;
    std::string title;
    std::string genre;
    std::string price;
    std::optional<std::string> units_sold;
};

struct GetBestCustomersRow {
    std::int64_t id;
    std::string name;
    std::string email;
    std::optional<std::string> total_spent;
};

inline const std::string SQL_CREATE_AUTHOR = R"sql(INSERT INTO author (name, bio, birth_year)
VALUES (?, ?, ?))sql";
inline const std::string SQL_GET_AUTHOR = R"sql(SELECT id, name, bio, birth_year
FROM author
WHERE id = ?)sql";
inline const std::string SQL_LIST_AUTHORS = R"sql(SELECT id, name, bio, birth_year
FROM author
ORDER BY name)sql";
inline const std::string SQL_UPDATE_AUTHOR_BIO = R"sql(UPDATE author SET bio = ? WHERE id = ?)sql";
inline const std::string SQL_DELETE_AUTHOR = R"sql(DELETE FROM author WHERE id = ?)sql";
inline const std::string SQL_CREATE_BOOK = R"sql(INSERT INTO book (author_id, title, genre, price, published_at)
VALUES (?, ?, ?, ?, ?))sql";
inline const std::string SQL_GET_BOOK = R"sql(SELECT id, author_id, title, genre, price, published_at
FROM book
WHERE id = ?)sql";
inline const std::string SQL_GET_BOOKS_BY_IDS = R"sql(SELECT id, author_id, title, genre, price, published_at
FROM book
WHERE id IN (SELECT value FROM JSON_TABLE(?,'$[*]' COLUMNS(value BIGINT PATH '$')) t)
ORDER BY title)sql";
inline const std::string SQL_LIST_BOOKS_BY_GENRE = R"sql(SELECT id, author_id, title, genre, price, published_at
FROM book
WHERE genre = ?
ORDER BY title)sql";
inline const std::string SQL_LIST_BOOKS_BY_GENRE_OR_ALL = R"sql(SELECT id, author_id, title, genre, price, published_at
FROM book
WHERE ? = 'all' OR genre = ?
ORDER BY title)sql";
inline const std::string SQL_CREATE_CUSTOMER = R"sql(INSERT INTO customer (name, email)
VALUES (?, ?))sql";
inline const std::string SQL_CREATE_SALE = R"sql(INSERT INTO sale (customer_id)
VALUES (?))sql";
inline const std::string SQL_ADD_SALE_ITEM = R"sql(INSERT INTO sale_item (sale_id, book_id, quantity, unit_price)
VALUES (?, ?, ?, ?))sql";
inline const std::string SQL_LIST_BOOKS_WITH_AUTHOR = R"sql(SELECT b.id, b.title, b.genre, b.price, b.published_at,
       a.name AS author_name, a.bio AS author_bio
FROM book b
JOIN author a ON a.id = b.author_id
ORDER BY b.title)sql";
inline const std::string SQL_GET_BOOKS_NEVER_ORDERED = R"sql(SELECT b.id, b.author_id, b.title, b.genre, b.price, b.published_at
FROM book b
LEFT JOIN sale_item si ON si.book_id = b.id
WHERE si.id IS NULL
ORDER BY b.title)sql";
inline const std::string SQL_GET_TOP_SELLING_BOOKS = R"sql(WITH book_sales AS (
    SELECT book_id,
           SUM(quantity) AS units_sold
    FROM sale_item
    GROUP BY book_id
)
SELECT b.id, b.title, b.genre, b.price,
       bs.units_sold
FROM book b
JOIN book_sales bs ON bs.book_id = b.id
ORDER BY bs.units_sold DESC)sql";
inline const std::string SQL_GET_BEST_CUSTOMERS = R"sql(WITH customer_spend AS (
    SELECT s.customer_id,
           SUM(si.quantity * si.unit_price) AS total_spent
    FROM sale s
    JOIN sale_item si ON si.sale_id = s.id
    GROUP BY s.customer_id
)
SELECT c.id, c.name, c.email,
       cs.total_spent
FROM customer c
JOIN customer_spend cs ON cs.customer_id = c.id
ORDER BY cs.total_spent DESC)sql";

void create_author(MYSQL* db, const std::string& name, const std::optional<std::string>& bio, const std::optional<std::int32_t>& birth_year);

std::optional<Author> get_author(MYSQL* db, const std::int64_t& id);

std::vector<Author> list_authors(MYSQL* db);

void update_author_bio(MYSQL* db, const std::optional<std::string>& bio, const std::int64_t& id);

void delete_author(MYSQL* db, const std::int64_t& id);

void create_book(MYSQL* db, const std::int64_t& author_id, const std::string& title, const std::string& genre, const std::string& price, const std::optional<std::string>& published_at);

std::optional<Book> get_book(MYSQL* db, const std::int64_t& id);

std::vector<Book> get_books_by_ids(MYSQL* db, const std::vector<std::int64_t>& ids);

std::vector<Book> list_books_by_genre(MYSQL* db, const std::string& genre);

std::vector<Book> list_books_by_genre_or_all(MYSQL* db, const std::string& genre);

void create_customer(MYSQL* db, const std::string& name, const std::string& email);

void create_sale(MYSQL* db, const std::int64_t& customer_id);

void add_sale_item(MYSQL* db, const std::int64_t& sale_id, const std::int64_t& book_id, const std::int32_t& quantity, const std::string& unit_price);

std::vector<ListBooksWithAuthorRow> list_books_with_author(MYSQL* db);

std::vector<Book> get_books_never_ordered(MYSQL* db);

std::vector<GetTopSellingBooksRow> get_top_selling_books(MYSQL* db);

std::vector<GetBestCustomersRow> get_best_customers(MYSQL* db);

class Querier {
    MYSQL* db_;
public:
    explicit Querier(MYSQL* db) : db_(db) {}

    void create_author(const std::string& name, const std::optional<std::string>& bio, const std::optional<std::int32_t>& birth_year);

    std::optional<Author> get_author(const std::int64_t& id);

    std::vector<Author> list_authors();

    void update_author_bio(const std::optional<std::string>& bio, const std::int64_t& id);

    void delete_author(const std::int64_t& id);

    void create_book(const std::int64_t& author_id, const std::string& title, const std::string& genre, const std::string& price, const std::optional<std::string>& published_at);

    std::optional<Book> get_book(const std::int64_t& id);

    std::vector<Book> get_books_by_ids(const std::vector<std::int64_t>& ids);

    std::vector<Book> list_books_by_genre(const std::string& genre);

    std::vector<Book> list_books_by_genre_or_all(const std::string& genre);

    void create_customer(const std::string& name, const std::string& email);

    void create_sale(const std::int64_t& customer_id);

    void add_sale_item(const std::int64_t& sale_id, const std::int64_t& book_id, const std::int32_t& quantity, const std::string& unit_price);

    std::vector<ListBooksWithAuthorRow> list_books_with_author();

    std::vector<Book> get_books_never_ordered();

    std::vector<GetTopSellingBooksRow> get_top_selling_books();

    std::vector<GetBestCustomersRow> get_best_customers();
};
