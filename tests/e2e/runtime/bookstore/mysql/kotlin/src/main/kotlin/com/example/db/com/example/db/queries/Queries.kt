...
object Queries {
    ...
    data class GetAuthorStatsRow(
        ...
        val name: String,
        val numBooks: Long,
        val totalSold: java.math.BigDecimal
    )
    ...
    fun getAuthorStats(conn: Connection): List<GetAuthorStatsRow> {
        conn.prepareStatement(SQL_GET_AUTHOR_STATS).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                while (rs.next()) rows.add(GetAuthorStatsRow(rs.getLong(1), rs.getString(2), rs.getLong(3), rs.getBigDecimal(4)))
            }
            return rows
        }
    }
    ...
    fun getProduct(conn: Connection, id: String): Product? {
        conn.prepareStatement(SQL_GET_PRODUCT).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                if (!rs.next()) return null
                return Product(rs.getString(1), rs.getString(2), rs.getString(3), rs.getBoolean(4), getNullableFloat(rs, 5), getNullableDouble(rs, 6), rs.getString(7), rs.getBytes(8), rs.getObject(9, java.time.LocalDateTime::class.java), rs.getShort(10))
            }
        }
    }
    ...
    fun listActiveProducts(conn: Connection, active: Boolean): List<ListActiveProductsRow> {
        conn.prepareStatement(SQL_LIST_ACTIVE_PRODUCTS).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                while (rs.next()) rows.add(ListActiveProductsRow(rs.getString(1), rs.getString(2), rs.getString(3), rs.getBoolean(4), getNullableFloat(rs, 5), getNullableDouble(rs, 6), rs.getString(7), rs.getObject(8, java.time.LocalDateTime::class.java), rs.getShort(9)))
            }
            return rows
        }
    }
    ...
    data class GetBooksWithSalesCountRow(
        ...
        val title: String,
        val genre: String,
        val totalQuantity: java.math.BigDecimal
    )
    ...
    fun getBooksWithSalesCount(conn: Connection): List<GetBooksWithSalesCountRow> {
        conn.prepareStatement(SQL_GET_BOOKS_WITH_SALES_COUNT).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                while (rs.next()) rows.add(GetBooksWithSalesCountRow(rs.getLong(1), rs.getString(2), rs.getString(3), rs.getBigDecimal(4)))
            }
            return rows
        }
    }
    ...
    data class GetSaleItemQuantityAggregatesRow(
        val minQty: Int?,
        val maxQty: Int?,
        val sumQty: java.math.BigDecimal?,
    ...
    fun getSaleItemQuantityAggregates(conn: Connection): GetSaleItemQuantityAggregatesRow? {
        conn.prepareStatement(SQL_GET_SALE_ITEM_QUANTITY_AGGREGATES).use { ps ->
            ps.executeQuery().use { rs ->
                if (!rs.next()) return null
                return GetSaleItemQuantityAggregatesRow(getNullableInt(rs, 1), getNullableInt(rs, 2), rs.getBigDecimal(3), rs.getBigDecimal(4))
            }
        }
    }
