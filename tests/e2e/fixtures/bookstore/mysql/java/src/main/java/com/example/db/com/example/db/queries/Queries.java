...
public final class Queries {
    ...
    private static final String SQL_UPDATE_AUTHOR_BIO = """
            ...
            """;
    public static void updateAuthorBio(Connection conn, String bio, long id) throws SQLException {
    ...
    private static final String SQL_DELETE_AUTHOR = """
            ...
            """;
    public static void deleteAuthor(Connection conn, long id) throws SQLException {
    ...
    public record GetTopSellingBooksRow(
        ...
        String genre,
        java.math.BigDecimal price,
        java.math.BigDecimal unitsSold
    ) {}
    ...
    public static List<GetTopSellingBooksRow> getTopSellingBooks(Connection conn) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(SQL_GET_TOP_SELLING_BOOKS)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) rows.add(new GetTopSellingBooksRow(rs.getLong(1), rs.getString(2), rs.getString(3), rs.getBigDecimal(4), rs.getBigDecimal(5)));
            }
            return rows;
        }
    }
    ...
    public record GetAuthorStatsRow(
        ...
        String name,
        long numBooks,
        java.math.BigDecimal totalSold
    ) {}
    ...
    public static List<GetAuthorStatsRow> getAuthorStats(Connection conn) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(SQL_GET_AUTHOR_STATS)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) rows.add(new GetAuthorStatsRow(rs.getLong(1), rs.getString(2), rs.getLong(3), rs.getBigDecimal(4)));
            }
            return rows;
        }
    }
    ...
    public static Optional<Product> getProduct(Connection conn, String id) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(SQL_GET_PRODUCT)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) return Optional.empty();
                return Optional.of(new Product(rs.getString(1), rs.getString(2), rs.getString(3), rs.getBoolean(4), getNullableFloat(rs, 5), getNullableDouble(rs, 6), rs.getString(7), rs.getBytes(8), rs.getObject(9, java.time.LocalDateTime.class), rs.getShort(10)));
            }
        }
    }
    ...
    public static List<ListActiveProductsRow> listActiveProducts(Connection conn, boolean active) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(SQL_LIST_ACTIVE_PRODUCTS)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) rows.add(new ListActiveProductsRow(rs.getString(1), rs.getString(2), rs.getString(3), rs.getBoolean(4), getNullableFloat(rs, 5), getNullableDouble(rs, 6), rs.getString(7), rs.getObject(8, java.time.LocalDateTime.class), rs.getShort(9)));
            }
            return rows;
        }
    }
    ...
    public static List<GetBooksWithSalesCountRow> getBooksWithSalesCount(Connection conn) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(SQL_GET_BOOKS_WITH_SALES_COUNT)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) rows.add(new GetBooksWithSalesCountRow(rs.getLong(1), rs.getString(2), rs.getString(3), rs.getBigDecimal(4)));
            }
            return rows;
        }
    }
    ...
    public static Optional<GetSaleItemQuantityAggregatesRow> getSaleItemQuantityAggregates(Connection conn) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(SQL_GET_SALE_ITEM_QUANTITY_AGGREGATES)) {
            try (ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) return Optional.empty();
                return Optional.of(new GetSaleItemQuantityAggregatesRow(getNullableInt(rs, 1), getNullableInt(rs, 2), rs.getBigDecimal(3), rs.getBigDecimal(4)));
            }
        }
    }
