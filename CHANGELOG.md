...
- **`sqltgen.org/install.sh` convenience URL** — a short, stable URL that
  ...
  `curl -fsSL https://sqltgen.org/install.sh | sh` is now the documented
  one-liner install for Linux and macOS; the full GitHub Release URL
  keeps working as before for users who prefer to pin to a specific tag.
