...
mod tests {
    ...
    fn test_apply_drop_tables_removes_matching() {
        let mut tables =
            vec![Table { name: "a".into(), columns: vec![] }, Table { name: "b".into(), columns: vec![] }, Table { name: "c".into(), columns: vec![] }];
    ...
    fn test_apply_drop_tables_ignores_nonexistent() {
        let mut tables = vec![Table { name: "a".into(), columns: vec![] }];
    ...
    fn test_apply_alter_table_unknown_table_is_noop() {
        let mut tables = vec![Table {
            name: "users".into(),
            columns: vec![Column { name: "id".into(), sql_type: SqlType::Integer, nullable: false, is_primary_key: true }],
        }];
