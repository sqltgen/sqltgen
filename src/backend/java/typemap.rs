...
impl JavaTypeMap {
    ...
    ///
    /// For `Array(inner)`, generates the full `java.util.Arrays.asList(...)` expression.
    pub(super) fn read_expr(&self, sql_type: &SqlType, nullable: bool, idx: usize) -> String {
        if let SqlType::Array(inner) = sql_type {
            return jdbc_array_read_expr(self.java_type_boxed(inner), nullable, idx);
        }
...
fn jdbc_array_read_expr(boxed_type: String, nullable: bool, idx: usize) -> String {
    if nullable {
        format!("rs.getArray({idx}) == null ? null : java.util.Arrays.asList(({boxed_type}[]) rs.getArray({idx}).getArray())")
    } else {
...
fn build_entry(defaults: &JavaTypeInfo, field_ov: Option<&ResolvedType>, param_ov: Option<&ResolvedType>) -> JavaTypeEntry {
    ...
    let (read, read_nullable) = resolve_read_exprs(defaults, field_ov);
    ...
    JavaTypeEntry { field_type, field_type_boxed, param_type, param_type_boxed, read, read_nullable, write, import, extra_fields }
}

/// Compute the `read` and `read_nullable` template strings for a type entry.
fn resolve_read_exprs(defaults: &JavaTypeInfo, field_ov: Option<&ResolvedType>) -> (String, String) {
    if let Some(resolved) = field_ov {
        if let Some(read_expr) = &resolved.read_expr {
            ...
            // Both nullable and non-nullable use the same expression (null guard is in helper).
            let read = read_expr.replace("{raw}", "rs.getString({idx})");
            return (read.clone(), read);
        }
...
fn java_type_info(sql_type: &SqlType) -> JavaTypeInfo {
    match sql_type {
        ...
        SqlType::Text | SqlType::Char(_) | SqlType::VarChar(_) | SqlType::Interval => {
            JavaTypeInfo { field_type: "String", field_type_boxed: "String", scalar_read: "rs.getString({idx})", nullable_helper: None, uses_get_object: false }
        },
        SqlType::Bytes => {
            JavaTypeInfo { field_type: "byte[]", field_type_boxed: "byte[]", scalar_read: "rs.getBytes({idx})", nullable_helper: None, uses_get_object: false }
        },
        ...
        SqlType::Array(_) | SqlType::Custom(_) => {
            JavaTypeInfo { field_type: "Object", field_type_boxed: "Object", scalar_read: "rs.getObject({idx})", nullable_helper: None, uses_get_object: false }
        },
    }
}
