...
fn build_queries_file_with_contract(
    ...
    let ds = schema.default_schema.as_deref();
    emit_table_imports(&mut src, &needed_tables(queries, schema), ds, &contract.output)?;
    emit_enum_imports(&mut src, &needed_enums(&queries.iter().collect::<Vec<_>>()), &contract.output)?;
    writeln!(src)?;
