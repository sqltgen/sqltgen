...
use crate::backend::common::{
    group_queries, has_inline_rows, infer_row_type_name, infer_table, needed_enums, querier_class_name, queries_file_stem,
...
pub(super) fn generate_core_files(
    ...
    for table in &schema.tables {
        ...
        files.push(GeneratedFile { path: PathBuf::from(&config.out).join("models").join(format!("{}.{ext}", table.name)), content });
    }
    ...
    if !schema.tables.is_empty() || !schema.enums.is_empty() {
        let stems: Vec<String> = schema.tables.iter().map(|t| t.name.clone()).chain(schema.enums.iter().map(|e| e.name.clone())).collect();
