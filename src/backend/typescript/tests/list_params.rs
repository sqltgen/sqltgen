...
fn sqlite_list_by_ids_query() -> Query {
    Query::many(
        "GetByIds",
        "SELECT id FROM t WHERE id IN ($1)",
        vec![Parameter::list(1, "ids", SqlType::BigInt, false)
...
fn mysql_list_by_ids_query() -> Query {
    Query::many(
        "GetByIds",
        "SELECT id FROM t WHERE id IN ($1)",
        vec![Parameter::list(1, "ids", SqlType::BigInt, false)
...
fn test_sqlite_dynamic_list_param_with_scalar_after() {
    ...
    let content = build_queries_file("", &[query], &schema, &JsTarget::BetterSqlite3, &JsOutput::TypeScript, &dynamic_cfg()).unwrap();
    assert!(content.contains("...ids, active"), "scalar after IN must follow list in args");
}
