...
fn test_generate_rust_helper_module_per_target() {
    ...
    assert!(get_file(&pg_files, "sqltgen.rs").contains("pub type DbPool = sqlx::PgPool;"));

    let sq_files = sqlite().generate(&schema, std::slice::from_ref(&query), &cfg()).unwrap();
    assert!(get_file(&sq_files, "sqltgen.rs").contains("pub type DbPool = sqlx::SqlitePool;"));

    let my_files = mysql().generate(&schema, std::slice::from_ref(&query), &cfg()).unwrap();
    assert!(get_file(&my_files, "sqltgen.rs").contains("pub type DbPool = sqlx::MySqlPool;"));
}
