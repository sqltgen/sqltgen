...
pub(super) fn generate_core_files(ctx: &GenerationContext) -> anyhow::Result<Vec<GeneratedFile>> {
    ...
    if !ctx.schema.tables.is_empty() || !ctx.schema.enums.is_empty() {
        ...
        write_all_list(&mut src, &exports)?;
    ...
    if !group_stems.is_empty() {
        ...
        write_all_list(&mut src, &group_stems)?;
    ...
    if !ctx.schema.tables.is_empty() || !ctx.schema.enums.is_empty() {
        ...
        exports.push("models".to_string());
    }
    if !group_stems.is_empty() {
        writeln!(src, "from . import queries")?;
        exports.push("queries".to_string());
    }
    write_all_list(&mut src, &exports)?;
...
/// Emit a `__all__ = [...]` re-export list. Each entry on its own line so
/// pyright's `reportUnusedImport` does not flag the imports above.
fn write_all_list(src: &mut String, names: &[String]) -> std::fmt::Result {
    if names.is_empty() {
        return Ok(());
    }
    writeln!(src)?;
