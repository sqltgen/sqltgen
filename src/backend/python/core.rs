...
use crate::config::{resolve_type_ref, ListParamStrategy, OutputConfig, ResolvedType, TypeVariant};
...
fn python_field_type(sql_type: &SqlType, nullable: bool, contract: &PythonCoreContract, config: &OutputConfig) -> String {
    ...
    python_type_with_json_mode(sql_type, nullable, contract.json_mode)
}
...
fn build_queries_file(group: &str, queries: &[Query], schema: &Schema, contract: &PythonCoreContract, config: &OutputConfig) -> anyhow::Result<String> {
    ...
    writeln!(src, "# Generated by sqltgen. Do not edit.")?;
    writeln!(src, "{}", contract.runtime_banner)?;
    writeln!(src, "from __future__ import annotations")?;
    writeln!(src)?;
    if any_inline_rows {
        writeln!(src, "import dataclasses")?;
    }
    writeln!(src, "{}", contract.db_import)?;
    ...
    imports.write(&mut src)?;
    writeln!(src)?;
    writeln!(src, "Connection = {}", contract.conn_type_expr)?;
    ...
    for query in queries {
        ...
        let sql = normalize_sql(&base_sql, contract.sql_norm_mode);
...
fn emit_python_list_query(src: &mut String, ctx: &PythonQueryContext, strategy: &ListParamStrategy, lp: &Parameter) -> anyhow::Result<()> {
    ...
    match strategy {
        ...
        ListParamStrategy::Dynamic => {
            ...
            let before = normalize_sql(&before_raw, ctx.contract.sql_norm_mode).replace('"', "\\\"").replace('\n', " ");
            let after = normalize_sql(&after_raw, ctx.contract.sql_norm_mode).replace('"', "\\\"").replace('\n', " ");
            let args_expr = build_dynamic_args(&scalar_params, lp);
            writeln!(src, "    placeholders = \", \".join([\"{}\"] * len({lp_name}))", ctx.contract.dynamic_placeholder_token)?;
