...
use crate::backend::common::{
    group_queries, has_inline_rows, infer_row_type_name, infer_table, needed_enums, querier_class_name, queries_file_stem, row_type_name, sql_const_name,
};
...
pub(super) fn generate_core_files(
    ...
    for table in &schema.tables {
        ...
        let path = PathBuf::from(&config.out).join("models").join(format!("{}.py", table.name));
    ...
    for e in &schema.enums {
        ...
        let path = PathBuf::from(&config.out).join("models").join(format!("{}.py", e.name));
        files.push(GeneratedFile { path, content: src });
    }
    ...
    for (group, group_queries) in &groups {
        ...
        let path = PathBuf::from(&config.out).join("queries").join(format!("{stem}.py"));
        files.push(GeneratedFile { path, content: src });
    }
    ...
    if !schema.tables.is_empty() || !schema.enums.is_empty() {
        ...
        let path = PathBuf::from(&config.out).join("models").join("__init__.py");
        files.push(GeneratedFile { path, content: src });
    }
    ...
    if !group_stems.is_empty() {
        ...
        let path = PathBuf::from(&config.out).join("queries").join("__init__.py");
        files.push(GeneratedFile { path, content: src });
    }
...
fn build_queries_file(
    ...
    if !needed_tables.is_empty() || !enum_names.is_empty() {
        ...
        for table_name in &needed_tables {
            ...
            writeln!(src, "from ..models.{table_name} import {class_name}")?;
        }
