...
pub(super) fn generate_core_files(
    ...
    for table in &schema.tables {
        ...
        let path = PathBuf::from(&config.out).join("models").join(format!("{file_stem}.py"));
...
fn build_queries_file(
    ...
    let mut needed_tables: Vec<_> = queries.iter().filter_map(|q| infer_table(q, schema)).collect();
