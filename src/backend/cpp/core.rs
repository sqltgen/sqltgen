...
fn emit_table_header(
    table: &Table,
    namespace: &str,
    cpp_type_fn: fn(&SqlType, bool) -> String,
    model_db_include: Option<&'static str>,
) -> anyhow::Result<String> {
    let mut src = String::new();
    let struct_name = to_pascal_case(&table.name);
    let stdlib_includes = scan_table_includes(table);

    // Emit the driver-specific db include only when at least one column uses a
    // non-stdlib type (detected by the absence of a `std::` prefix, e.g. `pqxx::bytes`).
    let driver_include = model_db_include.filter(|_| {
        table.columns.iter().any(|c| {
            let ty = cpp_type_fn(&c.sql_type, false);
            ty.contains("::") && !ty.starts_with("std::")
        })
    });
    ...
    for include in &stdlib_includes.set {
        writeln!(src, "#include {include}")?;
    }
    if let Some(inc) = driver_include {
        writeln!(src, "#include {inc}")?;
    }

    if !stdlib_includes.set.is_empty() || driver_include.is_some() {
        writeln!(src)?;
    }
    ...
    for col in &table.columns {
        writeln!(src, "    {} {};", cpp_type_fn(&col.sql_type, col.nullable), col.name)?;
    }
    writeln!(src, "}};")?;
...
pub(super) fn generate_table_files(
    schema: &Schema,
    config: &OutputConfig,
    cpp_type_fn: fn(&SqlType, bool) -> String,
    model_db_include: Option<&'static str>,
    ...
    for table in &schema.tables {
        files.push(GeneratedFile {
            ...
            content: emit_table_header(table, &config.package, cpp_type_fn, model_db_include)?,
        });
    }
...
fn cpp_param_decl_type(sql_type: &SqlType, nullable: bool, is_list: bool, cpp_type_fn: fn(&SqlType, bool) -> String) -> String {
    if is_list {
        return format!("const std::vector<{}>&", cpp_type_fn(sql_type, false));
    }

    let ty = cpp_type_fn(sql_type, nullable);
    if nullable {
...
fn params_signature(query: &Query, conn_type: &str, cpp_type_fn: fn(&SqlType, bool) -> String) -> String {
    let mut parts = vec![format!("{conn_type} db")];
    for p in &query.params {
        let decl_ty = cpp_param_decl_type(&p.sql_type, p.nullable, p.is_list, cpp_type_fn);
...
/// a known table.
fn emit_inline_row_struct(src: &mut String, query: &Query, cpp_type_fn: fn(&SqlType, bool) -> String) -> anyhow::Result<()> {
...
fn emit_function_decl(src: &mut String, query: &Query, schema: &Schema, conn_type: &str, cpp_type_fn: fn(&SqlType, bool) -> String) -> anyhow::Result<()> {
    let fn_name = to_snake_case(&query.name);
    let ret = query_return_type(query, schema);
    let params = params_signature(query, conn_type, cpp_type_fn);
...
fn emit_query_header_block(
    ...
    if has_inline_rows(query, schema) {
        emit_inline_row_struct(src, query, cpp_type_fn)?;
        writeln!(src)?;
    }
    emit_sql_constant(src, query, param_style)?;
    writeln!(src)?;
    emit_function_decl(src, query, schema, conn_type, cpp_type_fn)?;
    Ok(())
}
...
fn emit_function_def(src: &mut String, query: &Query, schema: &Schema, contract: &CppCoreContract) -> anyhow::Result<()> {
    let fn_name = to_snake_case(&query.name);
    let ret = query_return_type(query, schema);
    let params = params_signature(query, contract.conn_type, contract.cpp_type_fn);
    writeln!(src, "{ret} {fn_name}({params}) {{")?;
...
fn emit_querier_decl(
    ...
    for query in queries {
        ...
        let fn_name = to_snake_case(&query.name);
        let ret = query_return_type(query, schema);
        let params_no_db = querier_method_params(query, cpp_type_fn);
...
fn querier_method_params(query: &Query, cpp_type_fn: fn(&SqlType, bool) -> String) -> String {
...
fn emit_queries_header(group: &str, queries: &[Query], schema: &Schema, contract: &CppCoreContract, config: &OutputConfig) -> anyhow::Result<String> {
    ...
    for (i, query) in queries.iter().enumerate() {
        ...
        emit_query_header_block(&mut src, query, schema, contract.param_style, contract.conn_type, contract.cpp_type_fn)?;
    }

    // Querier class.
    if !queries.is_empty() {
        writeln!(src)?;
        emit_querier_decl(&mut src, group, queries, schema, contract.conn_type, contract.cpp_type_fn)?;
    }
...
fn emit_queries_source(
    ...
    if !queries.is_empty() {
        writeln!(src)?;
        emit_querier_method_defs(&mut src, group, queries, schema, &config.package, contract.cpp_type_fn)?;
    }
