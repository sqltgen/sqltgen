...
`make test` in any combo project regenerates the sqltgen output, then
diffs it against the committed snapshot. Behavior depends on the
environment:

| Mode | Diff state | Behavior |
|------|------------|----------|
| Local | clean | skip runtime (snapshot trusted) |
| Local | dirty | run runtime |
| CI (`CI=1`) | clean | run runtime |
| CI (`CI=1`) | dirty | fail-fast (don't run runtime) |
| `FORCE_RUNTIME=1` | any | always run runtime |

Locally, this means `make test` in an unchanged combo finishes in
~seconds — the runtime suite only fires when codegen output actually
changed. On CI, drift fails the build immediately and prints the diff;
the PR author must regenerate locally, verify with runtime, and commit
the new output before pushing again.

Use `FORCE_RUNTIME=1 make test` when you want to bypass the gate
(e.g. testing a driver/DB upgrade that doesn't change generated bytes).
